...
public class BeanCondicionComercial4x {
		...
		if(listEscalas!=null && listEscalas.size()>0){
			...
			for (BeanCondicionComercialEscala beanEscala : listEscalas) {
				if(beanEscala.getStrTipoEscala().equals("V")){
					dblValor = new Double(beanEscala.getStrValorEscala());
				}
				else{
					dblValor = new Double(beanEscala.getStrCantEscala());
				}
				
				if(dblValor.compareTo(dblLimiteInferior)>=0 && dblValor.compareTo(dblLimiteInferior)<=0){
	...
	public Double getDblImporteEscala(Double dblCompara) {
		...
		if(listEscalas!=null && listEscalas.size()>0){
			...
			for (BeanCondicionComercialEscala beanEscala : listEscalas) {
				...
				if(dblCompara.compareTo(dblLimiteInferior)>0 && dblCompara.compareTo(new Double(beanEscala.getStrCantEscala()))<=0){
					dblRetorno = new Double(beanEscala.getStrImporte());
					//dblLimiteInferior = new Double(beanEscala.getStrValorEscala());
				}
				else{
