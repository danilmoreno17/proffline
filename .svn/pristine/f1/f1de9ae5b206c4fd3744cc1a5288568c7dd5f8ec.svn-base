...
public class IIndicadoresVendedor extends javax.swing.JInternalFrame {
	...
	private void cargarReporteIndicadores() {
		...
		if (usuario.getStrModo().equals("1")) {
			...
			Thread hilo = new Thread() {
				public void run() {
					...
					String[] nombresColumnas = { "CÓDIGO KPI", "DESCRIPCIÓN", "REAL", "OBJETIVO", "CUMPLIMIENTO" };
		...
		Mensaje.mostrarAviso("Consulta Indicador Vendedor Requiere Conexión Online");
		}			
	}
	...
	private void cargarReporteDetalle() {
		...
		Thread hilo = new Thread() {
			public void run() {
				...
				String[] nombresColumnas = { "CÓD. CLI.", "CLI.", "PICKING", "VEN. NETA", "COBRZA. REAL", "COBRZA OBJ." };
